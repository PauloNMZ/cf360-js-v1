...
const EmpresaCards: React.FC<EmpresaCardsProps> = ({
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
      {convenentes.map(convenente => <div key={convenente.id} className={`bg-gradient-to-br from-gray-900 to-gray-800 dark:from-gray-800 dark:to-gray-900 rounded-xl p-6 border-2 transition-all duration-300 hover:shadow-xl hover:scale-105 cursor-pointer flex flex-col ${currentConvenenteId === convenente.id ? 'border-blue-500 shadow-lg shadow-blue-500/20' : 'border-gray-600 hover:border-gray-500'}`} onClick={() => onSelectConvenente(convenente, 'view')}>
          ...
          backgroundColor: '#5D5E6A'
        }} className="hover:opacity-90 text-white border-0 px-6 py-2 text-xs font-medium transition-all" onClick={e => {
          e.stopPropagation();
          ...
          {currentConvenenteId === convenente.id && <div className="absolute top-2 right-2">
