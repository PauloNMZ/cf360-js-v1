...
export class GeradorCNAB240 {
  ...
  private gerarHeaderArquivo(): void {
    ...
    header += "1";                                                           // Remittance/return code (1=Remittance)
  ...
  private gravarSegmentoA(
    ...
    // Control
    segmentoA += ajustarTamanho(COD_BB, 3);                                  // Bank code
    ...
    if (tipoLancamento === "03") {
      ...
      segmentoA += ajustarTamanho(COD_BB, 3);                                // Bank code (BB)
    }
    ...
    segmentoA += ajustarTamanho(this.config.nrDocumento, 20);                // Client document number
    
    // Payment date
    segmentoA += ajustarTamanho(dataPagamento, 8);                           // Payment date
    
    // Currency type
    segmentoA += ajustarTamanho(MOEDA_BRL, 3);                               // Currency type (BRL)
    ...
    segmentoA += ajustarTamanho("", 20);                                     // Our number - Doc number assigned by bank (blank)
    
    // Effective date
    segmentoA += ajustarTamanho("0", 8, "0", true);                          // Effective payment date (Return file = DDMMYYYY)
    
    // Effective value
    segmentoA += ajustarTamanho("0", 15, "0", true);                         // Effective payment value (Return file = Payment value made)
    ...
    // Occurrences
    segmentoA += ajustarTamanho("0", 10, "0");                               // Final complement - Remittance file = '0000000000'
    
    // Ensure record has exactly 240 characters
    segmentoA = ajustarTamanho(segmentoA, 240);
  ...
  private gravarSegmentoB(
    ...
    // Control
    segmentoB += ajustarTamanho(COD_BB, 3);                                               // Bank code
    ...
    // Ensure record has exactly 240 characters
    segmentoB = ajustarTamanho(segmentoB, 240);
